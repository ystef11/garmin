...
ЗАПУСК
  ...
  python garmin_plan_import.py plan.json               # весь план, НО по умолчанию только даты >= сегодня
  python garmin_plan_import.py plan.json --all-dates    # весь план целиком, включая прошедшие даты
  python garmin_plan_import.py plan.json --from 2026-08-01  # загрузка начиная с указанной даты
  python garmin_plan_import.py plan.json --no-clear-existing  # не чистить дубли перед загрузкой

ВАЖНО: по умолчанию (без --all-dates) в Garmin загружаются только тренировки
с датой >= сегодня — прошедшие тренировки из plan.json пропускаются. Это не
влияет на --clear/--clear-past (они всегда смотрят на весь план).

ВАЖНО: по умолчанию (без --no-clear-existing) перед созданием тренировок сначала
удаляются уже существующие в Garmin тренировки с такими же именами, что и
загружаемые сейчас — так повторный запуск (после правки плана или по ошибке)
не плодит дубли. Затрагивает только те тренировки, что реально сейчас
создаются (после фильтра по датам/--test), а не весь план с тегом.
"""
...
# Отключаем систему плагинов pydantic (её пытается подхватить пакет logfire,
# если он установлен в окружении) — плагин на инициализации вызывает
# inspect.getsource(), который падает с "could not get source code" в
# собранном PyInstaller-exe (там нет исходников .py, только байткод).
# Должно быть выставлено ДО первого "import garth" (garth тянет pydantic.dataclasses).
...
def run_import(plan, dry_run=False, test=False, clear=False, clear_past=False,
               ...
               all_dates=False, from_date=None, no_clear_existing=False):
    ...
    По умолчанию (all_dates=False) в Garmin загружаются только тренировки с
    датой >= from_date (по умолчанию — сегодня); это относится только к
    созданию тренировок (--clear/--clear-past всегда работают по всему плану).

    По умолчанию (no_clear_existing=False) перед созданием новых тренировок
    сначала удаляются уже существующие в Garmin тренировки с ТАКИМИ ЖЕ именами
    (тег плана + номер недели/дня — уникальны для конкретной тренировки плана) —
    это защищает от дублей при повторном запуске (например, план чуть
    поменялся и его перезаливают, или скрипт запускают ещё раз по ошибке).
    Затрагивает только те тренировки, что сейчас реально будут создаваться
    (после фильтра по датам/--test), а не весь план — используй --clear/
    --clear-past, если нужно почистить весь план целиком. Отключить —
    --no-clear-existing (тогда просто выводится предупреждение о дублях, как
    раньше).
    """
    tag, all_items, meta = load_plan(plan, (skip_cross or '').split(','))
    items = all_items

    if not (clear or clear_past) and not all_dates:
        try:
            cutoff_from = datetime.date.fromisoformat(from_date) if from_date else datetime.date.today()
        except ValueError:
            sys.exit(f"Неверная дата в --from: {from_date} (нужен формат ГГГГ-ММ-ДД)")
        before_filter = len(items)
        items = [x for x in items if x[0] >= cutoff_from]
        skipped_past = before_filter - len(items)
        if skipped_past:
            print(f"Пропущено прошедших тренировок (дата < {cutoff_from.isoformat()}): {skipped_past} "
                  f"(используй --all-dates, чтобы загрузить весь план)")
        if not items:
            sys.exit(f"После фильтра по дате (>= {cutoff_from.isoformat()}) в плане не осталось тренировок. "
                     "Используй --all-dates или --from ГГГГ-ММ-ДД.")
        ...
        try:
            ...
            if existing:
                print(f"ВНИМАНИЕ: в Garmin уже есть {len(existing)} тренировок с тегом {tag} — "
                      f"повторный импорт создаст дубли (--no-clear-existing отключил автоочистку).\n")
        except Exception:
            pass
    else:
        try:
            names_to_upload = {name for _, name, _ in items}
            lst = get("/workout-service/workouts?start=0&limit=999") or []
            dupes = [w for w in lst if w.get("workoutName") in names_to_upload]
        except Exception as e:
            dupes = []
            print(f"Не удалось проверить существующие тренировки перед загрузкой ({e}) — продолжаю без автоочистки.")
        if dupes:
            print(f"Удаляю {len(dupes)} уже существующих тренировок с такими же именами "
                  f"(чтобы не плодить дубли; --no-clear-existing — отключить)...")
            removed_dupes = 0
            for w in dupes:
                try:
                    delete(f"/workout-service/workout/{w['workoutId']}")
                    ...
                    print(f"  FAIL удаления дубля: {w.get('workoutName')} -> {e}")
                time.sleep(0.2)
...
def main():
    ...
    ap.add_argument("--all-dates", action="store_true",
                    help="загрузить весь план целиком, включая прошедшие даты "
                         "(по умолчанию загружаются только тренировки с датой >= сегодня)")
    ap.add_argument("--from", dest="from_date", metavar="ГГГГ-ММ-ДД",
                    help="загрузить тренировки начиная с этой даты (по умолчанию — сегодня)")
    ap.add_argument("--no-clear-existing", action="store_true",
                    help="не удалять перед загрузкой уже существующие тренировки с такими же "
                         "именами (по умолчанию удаляются автоматически — иначе повторный запуск "
                         "создаёт дубли)")
    args = ap.parse_args()
