...
ОФП/силовые — отдельными тренировками strength_training. К каждой тренировке добавляется
комментарий (описание) и подписи к шагам (видны на часах). Дни отдыха не создаются.
...
WEEKS = [
 # Н1 — Пиковая (текущая)
 [ [],
   [RUN("Интервалы 6x1000", warm(15)+reps(6,1000,RI515,90)+cool()), SA()],
   [RUN("Лёгкий", easy(55,6))],
   [RUN("Лёгкий", easy(45))],
   [],
   [RUN("Порог 2x15'", warm(12)+tmin(2,15,RTHR,3)+cool()), SB()],
   [RUN("Длинная 1:55", long_easy(115))] ],
 # Н2 — Разгрузка (после пика)
 [ [],
   [RUN("Освежающие 6x400", warm(12)+reps(6,400,RREP,90)+cool(8)), SA()],
   [RUN("Лёгкий", easy(40,5))],
   [RUN("Лёгкий", easy(35))],
   [],
   [RUN("Порог 2x8'", warm(12)+tmin(2,8,RTHR,2)+cool(8)), SB()],
   [RUN("Длинная 1:25 лёгкий", long_easy(85))] ],
 # Н3 — Развитие
 [ [],
   [RUN("Интервалы 5x1000", warm(15)+reps(5,1000,RI515,90)+cool()), SA()],
   [RUN("Лёгкий", easy(50,6))],
   [RUN("Лёгкий", easy(45))],
   [],
   [RUN("Порог 2x12'", warm(12)+tmin(2,12,RTHR,3)+cool()), SB()],
   [RUN("Длинная 1:45", long_easy(105))] ],
 # Н4 — Развитие
 [ [],
   [RUN("Интервалы 6x1000", warm(15)+reps(6,1000,RI510,90)+cool()), SA()],
   [RUN("Лёгкий", easy(55,6))],
   [RUN("Лёгкий", easy(45))],
   [],
   [RUN("Порог 2x15'", warm(12)+tmin(2,15,RTHR,3)+cool()), SB()],
 ...
 [ [],
   [RUN("Темп10к 5x1200", warm(15)+reps(5,1200,RI520,90)+cool()), SA()],
   [RUN("Лёгкий", easy(55,6))],
   [RUN("Лёгкий", easy(45))],
   [],
   [RUN("Темп 25' непрерывно", warm(12)+thr_cont(25,RTHR)+cool()), SB()],
   [RUN("Длинная 2:00 ПИК", long_easy(120))] ],
 # Н6 — Разгрузка
 [ [],
   [RUN("Освежающие 8x400", warm(12)+reps(8,400,RREP2,90)+cool(8)), SA()],
 ...
 # Н7 — Специфика 10К
 [ [],
   [RUN("Темп10к 5x1000", warm(15)+reps(5,1000,RI515,90)+cool()), SA()],
   [RUN("Лёгкий", easy(55,6))],
   [RUN("Лёгкий", easy(45))],
   [],
   [RUN("Порог 3x10'", warm(12)+tmin(3,10,RTHR,2)+cool()), SB()],
   [RUN("Длинная 1:50", long_easy(110))] ],
 # Н8 — Специфика 10К
 [ [],
   [RUN("Темп10к 6x1000", warm(15)+reps(6,1000,RI510,90)+cool()), SA()],
   [RUN("Лёгкий", easy(55,6))],
   [RUN("Лёгкий", easy(45))],
   [],
   [RUN("Порог 2x15'", warm(12)+tmin(2,15,RTHR2,3)+cool()), SB()],
   [RUN("Длинная 1:50", long_easy(110))] ],
 # Н9 — Заострение
 [ [],
   [RUN("Темп10к 5x1000", warm(15)+reps(5,1000,RI510,90)+cool()), SA()],
   [RUN("Лёгкий", easy(50,6))],
   [RUN("Лёгкий", easy(45))],
   [],
   [RUN("Порог 2x12'", warm(12)+tmin(2,12,RTHR2,3)+cool()), SB()],
   [RUN("Длинная 1:40", long_easy(100))] ],
 # Н10 — Заострение (силовая снижается)
 [ [],
   [RUN("Темп10к 5x1000", warm(15)+reps(5,1000,RI510s,90)+cool()), SAL()],
